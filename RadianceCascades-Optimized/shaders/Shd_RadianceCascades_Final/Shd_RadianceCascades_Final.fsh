...
vec4 raymarch(vec2 origin, vec2 delta, float interval) {
	...
	for(float ii = 0.0, dd = 0.0, rr = 0.0, ee = 0.00001; ii < interval; ii++) {
		vec2 ray = (origin + (delta * rr)) * (1.0 / in_RenderExtent);
		dd = V2F16(texture2D(in_DistanceField, ray).rg);
		rr += dd * scale;
...
vec4 merge(vec4 radiance, float index, vec2 extent, vec2 probe) {
	...
	vec4 interpolated = texture2D(gm_BaseTexture, (interpN1 + (probe * 0.5) + 0.25) * (1.0 / in_CascadeExtent));
	return radiance + interpolated;
}
...
void main() {
	...
	vec2 linear = (probe.xy + 0.5) * (in_CascadeLinear * pow(2.0, in_CascadeIndex));
	...
	for(float i = 0.0; i < 4.0; i++) {
		...
		vec2 ray = linear + (delta * interval);
		vec4 radiance = raymarch(ray, delta, limit);
		gl_FragColor += merge(radiance, preavg, extent, probe.xy) * 0.25;
	}
	
	if (in_CascadeIndex == 0.0) gl_FragColor = vec4(LINEAR(gl_FragColor), 1.0);
}